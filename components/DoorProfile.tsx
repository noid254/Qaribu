...
import { WhatsAppIcon, BackIcon, BedIcon, BathIcon, PhoneIcon, UserPlusIcon, MenuIcon } from './Icons';
        ...
        <div className="bg-surface-muted min-h-screen font-sans flex flex-col relative">
            ...
            <div className="relative h-80 bg-gray-900">
                ...
                <div className="absolute bottom-8 left-6 text-white z-10 max-w-[80%]">
                    ...
                    <p className="text-body opacity-90 font-medium mt-1 text-brand-gold">{premise.name}, {unit.floor} Floor</p>
                    
                    {isOccupied && tenant && (
                         <div className="flex items-center gap-1 mt-2 text-yellow-400 text-body font-bold">
            ...
            {/* Content Body */}
            <div className="flex-1 -mt-6 bg-white rounded-t-sheet relative z-10 px-6 pt-8 pb-28 shadow-[0_-10px_40px_rgba(0,0,0,0.1)]">
                        ...
                        <div className="flex gap-6">
                            {unit.bedrooms && (
                                <div className="flex items-center gap-2 text-ink-soft">
                                    <BedIcon />
                                    <span className="text-body font-bold">{unit.bedrooms} <span className="font-normal text-caption">Bed</span></span>
                                </div>
                            )}
                            {unit.bathrooms && (
                                <div className="flex items-center gap-2 text-ink-soft">
                                    <BathIcon />
                                    <span className="text-body font-bold">{unit.bathrooms} <span className="font-normal text-caption">Bath</span></span>
                                </div>
                            )}
                            {unit.size && (
                                <div className="flex items-center gap-2 text-ink-soft">
                                    <ExpandIcon />
                                    <span className="text-body font-bold">{unit.size}</span>
                                </div>
                            )}
                        </div>
                        {isVacant && (
                            <div className="text-right">
                                ...
                                <span className="text-caption text-ink-soft font-medium">{periodDisplay}</span>
                            </div>
                        )}
                    </div>
                )}
                ...
                <div className="mb-6">
                    ...
                    <p className="text-ink-soft text-body leading-relaxed whitespace-pre-line">
