...
const HostSetup: React.FC<HostSetupProps> = ({ setupData, currentUser, onSave, onBack }) => {
    ...
    if (setupData.role === 'Gateman') {
        return (
            <div className="bg-surface-muted min-h-screen font-sans flex flex-col items-center justify-center p-6 text-center">
                <div className="w-24 h-24 bg-info-soft rounded-full flex items-center justify-center mb-6 text-info">
                ...
                <button onClick={onBack} className="mt-4 text-ink-soft font-bold text-sm">Cancel</button>
            </div>
        );
    }
    ...
    if (setupData.role === 'Staff') {
        return (
            <div className="bg-surface-muted min-h-screen font-sans flex flex-col items-center justify-center p-6 text-center">
                <div className="w-24 h-24 bg-success-soft rounded-full flex items-center justify-center mb-6 text-success">
                ...
                <button onClick={handleSubmit} className="w-full bg-success text-white font-bold py-4 rounded-xl shadow-lg hover:bg-success-strong transition active:scale-95">
                    Accept Invitation
                </button>
                <button onClick={onBack} className="mt-4 text-ink-soft font-bold text-sm">Decline</button>
            </div>
        );
    }
    ...
    return (
        <div className="bg-surface-muted min-h-screen flex flex-col font-sans">
            <header className="bg-white shadow-sm p-4 sticky top-0 z-10 flex items-center gap-4">
                ...
                <h1 className="text-xl font-bold text-ink">Setup Your Unit</h1>
            </header>

            <main className="flex-1 p-4 pb-24 overflow-y-auto">
                <div className="bg-white p-4 rounded-xl shadow-sm border border-line mb-6">
                    <h2 className="text-sm font-bold text-ink-soft uppercase mb-2">Location</h2>
                    <p className="text-lg font-bold text-brand-navy">Unit {setupData.unitId}</p>
                    <p className="text-sm text-ink-soft">Premise ID: {setupData.premiseId}</p>
                </div>

                <div className="mb-6">
                    <h2 className="text-lg font-bold text-ink mb-3">I am setting up as a...</h2>
                    <div className="flex gap-3">
                        <button 
                            onClick={() => setType('Business')}
                            className={`flex-1 py-4 rounded-xl border-2 font-bold transition-all ${type === 'Business' ? 'border-brand-navy bg-info-soft text-brand-navy' : 'border-line bg-white text-ink-soft'}`}
                        >
                            Business
                        </button>
                        <button 
                            onClick={() => setType('Residence')}
                            className={`flex-1 py-4 rounded-xl border-2 font-bold transition-all ${type === 'Residence' ? 'border-brand-navy bg-info-soft text-brand-navy' : 'border-line bg-white text-ink-soft'}`}
                        >
                            Residence
                        </button>
                    </div>
                ...
                {type === 'Business' && (
                    <div className="space-y-6 animate-fade-in">
                        <div className="bg-white p-4 rounded-xl shadow-sm border border-line">
                            <h3 className="font-bold text-ink mb-3">Current Status</h3>
                            <select 
                                value={status} 
                                onChange={e => setStatus(e.target.value as any)}
                                className="w-full p-3 border rounded-lg bg-surface-muted font-semibold text-ink focus:outline-none focus:ring-2 focus:ring-brand-gold"
                            >
                        ...
                        <div className="bg-white p-4 rounded-xl shadow-sm border border-line">
                            ...
                            <div className="flex items-center gap-2">
                                <input type="time" value={startTime} onChange={e => setStartTime(e.target.value)} className="flex-1 p-3 border rounded-lg bg-surface-muted" />
                                <span className="text-ink-faint">to</span>
                                <input type="time" value={endTime} onChange={e => setEndTime(e.target.value)} className="flex-1 p-3 border rounded-lg bg-surface-muted" />
                            </div>
                        </div>
                        ...
                        <div className="bg-white p-4 rounded-xl shadow-sm border border-line">
                            ...
                            <div className="flex items-center gap-3 p-3 bg-surface-muted rounded-lg border border-line">
                                ...
                                <div>
                                    ...
                                    <p className="text-xs text-ink-soft">{currentUser.service}</p>
                                </div>
                            </div>
                            <p className="text-xs text-ink-soft mt-2">Visitors will see your NikoSoko profile details when they scan your door.</p>
                        </div>
                    </div>
                )}
                ...
                {type === 'Residence' && (
                    <div className="space-y-6 animate-fade-in">
                        ...
                        <div 
                            ...
                            className={`p-4 rounded-xl border-2 flex items-center justify-between cursor-pointer transition-all duration-300 ${status === 'Do Not Disturb' ? 'border-danger bg-danger-soft' : 'border-success bg-success-soft'}`}
                        >
                            <div className="flex items-center gap-3">
                                ...
                                <div>
                                    <h3 className="font-bold text-ink">{status === 'Do Not Disturb' ? 'Do Not Disturb' : 'Available'}</h3>
                                    <p className="text-xs text-ink-soft">{status === 'Do Not Disturb' ? 'Visitors cannot ring doorbell' : 'Visitors can ring doorbell'}</p>
                                </div>
                            </div>
                            <div className={`w-12 h-7 rounded-full p-1 transition-colors duration-300 flex items-center ${status === 'Do Not Disturb' ? 'bg-danger' : 'bg-success'}`}>
