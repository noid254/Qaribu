    ...
    return (
        <div className="bg-surface-sunken min-h-screen font-sans">
            ...
            <header className="bg-white shadow-sm p-4 sticky top-0 z-10 flex justify-between items-center">
                <button onClick={onBack} className="text-ink-soft font-semibold text-sm">Back to Dashboard</button>
                <h1 className="font-bold text-lg text-ink">Manage Order</h1>
                <div className={`px-3 py-1 rounded-full text-xs font-bold uppercase ${status === 'Pending' ? 'bg-warning-soft text-warning-strong' : 'bg-success-soft text-success-strong'}`}>
                    {status}
                </div>
            </header>
            ...
            <div className="p-4 space-y-6 max-w-lg mx-auto">
                ...
                <div className="bg-white rounded-xl shadow-md p-5 border-l-4 border-brand-navy">
                    <div className="flex justify-between items-start">
                        <div>
                            <p className="text-xs text-ink-soft font-bold uppercase mb-1">Customer</p>
                ...
                <div className="bg-white rounded-xl shadow-sm p-5">
                    <p className="text-xs text-ink-soft font-bold uppercase mb-3 border-b pb-2">Order Details • {new Date(orderData.date).toLocaleTimeString()}</p>
                    <div className="space-y-3 mb-4">
                        {orderData.items.map((item, idx) => (
                            <div key={idx} className="flex justify-between items-center">
                                <div className="flex items-center gap-3">
                                    <span className="bg-surface-sunken text-ink-soft font-bold text-xs w-6 h-6 flex items-center justify-center rounded">{item.qty}x</span>
                                    <span className="text-ink font-medium text-sm">{item.name}</span>
                                </div>
                                <span className="text-ink-soft text-sm">Ksh {item.price.toLocaleString()}</span>
                            </div>
                        ))}
                    </div>
                    ...
                    {nearbyRiders.length > 0 ? (
                        <div className="space-y-3">
                            {nearbyRiders.map(rider => (
                                <div key={rider.id} className="bg-white rounded-xl p-4 shadow-sm border border-line flex items-center justify-between">
                                    <div className="flex items-center gap-3">
                                        <img src={rider.avatarUrl} alt={rider.name} className="w-12 h-12 rounded-full object-cover bg-surface-sunken" />
                                        <div>
                                            <p className="font-bold text-ink text-sm">{rider.name}</p>
                                            <p className="text-xs text-ink-soft">{rider.distanceKm}km away • ⭐ {rider.rating}</p>
                                        </div>
                                    </div>
                                    <button 
                                        onClick={() => handleDispatch(rider)}
                                        className="bg-success-soft text-success-strong hover:bg-success hover:text-white border border-success-soft font-bold text-xs px-3 py-2 rounded-lg transition flex items-center gap-1"
                                    >
                        ...
                        <div className="bg-white rounded-xl p-6 text-center border border-dashed">
                            <p className="text-ink-soft text-sm">No registered riders found nearby.</p>
                        </div>
                    )}
                </div>
            </div>
