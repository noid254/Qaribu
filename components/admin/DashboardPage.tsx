...
import { UserPlusIcon as SharedUserPlusIcon, UsersIcon as SharedUsersIcon } from '../Icons';

const UsersIcon = () => <SharedUsersIcon className="h-8 w-8" />;
const UserPlusIcon = () => <SharedUserPlusIcon className="h-8 w-8" />;
...
const StatCard: React.FC<{ title: string, value: string, icon: React.ReactNode, change: string, onClick?: () => void }> = ({ title, value, icon, change, onClick }) => (
    <div onClick={onClick} className={`bg-white p-6 rounded-Nonecontrol shadow ${onClick ? 'cursor-pointer hover:shadow-md hover:-translate-y-1 transition-transform' : ''}`}>
        ...
        <p className="text-body text-success mt-2">{change}</p>
    </div>
);
