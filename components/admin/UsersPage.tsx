                ...
                {filteredProviders.map(p => (
                    <div key={p.id} className="flex items-center justify-between p-3 bg-surface-muted rounded-lg">
                        ...
                        <div className="flex items-center space-x-2">
                            {!p.isVerified && <button onClick={() => onUpdateProvider({...p, isVerified: true})} className="text-xs bg-success-soft text-success-strong px-2 py-1 rounded">Verify</button>}
                            <button onClick={() => {if(window.confirm(`Delete ${p.name}?`)) onDeleteProvider(p.id)}} className="text-xs bg-danger-soft text-danger-strong px-2 py-1 rounded">Delete</button>
                        </div>
                    </div>
                ))}
