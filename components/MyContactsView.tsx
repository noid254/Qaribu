            ...
            <div className="p-4 space-y-3">
                {contacts.length > 0 ? (
                    contacts.map(contact => (
                        <button key={contact.id} onClick={() => onSelectContact(contact)} className="w-full text-left bg-white p-3 rounded-xl shadow-sm hover:shadow-md transition-shadow flex items-center gap-4">
                            ...
                            <div>
                                ...
                                <p className="text-sm text-ink-soft">{contact.service}</p>
                            </div>
                        </button>
                    ))
                ) : (
                    <div className="text-center py-16 text-ink-soft">
                        ...
                        <h3 className="mt-2 text-sm font-medium text-ink">No Contacts Yet</h3>
                        <p className="mt-1 text-sm text-ink-soft">You haven't saved any contacts yet.</p>
                    </div>
                )}
            </div>
        </div>
