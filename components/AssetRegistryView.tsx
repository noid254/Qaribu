    ...
    return (
        <button onClick={onClick} className="w-full text-left bg-white p-3 rounded-xl shadow-sm hover:shadow-md transition-shadow flex items-center gap-4 border">
            <img 
                ...
                alt={assetName}
                className="w-16 h-16 rounded-lg object-cover flex-shrink-0 bg-surface-sunken"
            />
            <div className="flex-grow overflow-hidden">
                <p className="font-bold text-ink truncate">{assetName}</p>
                <p className="text-xs font-mono text-ink-soft mt-1">{assetIdentifier}</p>
    ...
    return (
        <div className="p-4 bg-surface-muted min-h-full space-y-6">
            <div className="bg-white p-4 rounded-xl shadow-sm space-y-3 border">
                ...
                <p className="text-sm text-ink-soft">Verify the ownership of a tool or vehicle before you buy by checking its serial or registration number.</p>
                <button onClick={() => onNavigate('ownershipCheck')} className="w-full bg-info text-white font-bold py-3 px-4 rounded-lg">
                    Check an Asset
                </button>
            </div>
            ...
            <div>
                ...
                {myAssets.length > 0 ? (
                    <div className="space-y-3">
                     ...
                     <div className="text-center py-10 text-ink-soft bg-white rounded-xl border">
                        ...
                        <h3 className="mt-2 text-sm font-medium text-ink">No Assets Registered</h3>
                        <p className="mt-1 text-sm text-ink-soft">Register your tools and vehicles to protect them.</p>
                    </div>
                )}
            </div>
        </div>
