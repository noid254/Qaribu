...
const InputField: React.FC<{ label: string, value: string, onChange: (e: React.ChangeEvent<HTMLInputElement | HTMLTextAreaElement>) => void, placeholder: string, required?: boolean, isTextarea?: boolean }> = ({ label, value, onChange, placeholder, required = false, isTextarea = false }) => (
    <div>
        ...
        {isTextarea ? (
            <textarea value={value} onChange={onChange} placeholder={placeholder} required={required} rows={4} className="w-full p-3 border-2 border-line rounded-lg bg-surface-muted focus:bg-white focus:ring-2 focus:ring-brand-gold focus:outline-none transition" />
        ) : (
            <input value={value} onChange={onChange} placeholder={placeholder} required={required} className="w-full p-3 border-2 border-line rounded-lg bg-surface-muted focus:bg-white focus:ring-2 focus:ring-brand-gold focus:outline-none transition" />
        )}
    </div>
);
                ...
                return (
                    <form onSubmit={handleNext} className="space-y-6">
                        ...
                        <div>
                            ...
                            <div onClick={() => logoInputRef.current?.click()} className="cursor-pointer w-full h-40 border-2 border-dashed rounded-lg flex flex-col items-center justify-center bg-surface-muted hover:bg-surface-sunken transition-colors">
                                {logo ? (
                ...
                return (
                     <form onSubmit={handleNext} className="space-y-6">
                        ...
                        <div className="flex gap-3">
                            <button type="button" onClick={handleBack} className="flex-1 bg-surface-sunken text-ink font-bold py-4 rounded-xl">Back</button>
                ...
                return (
                    <div className="space-y-6">
                        <div className="space-y-4 text-sm">
                            <h3 className="font-bold text-ink-soft uppercase text-xs">Review Details</h3>
                            <div className="flex justify-between items-center p-3 bg-surface-muted rounded-lg"><span>Business Name</span><span className="font-semibold text-right">{businessName}</span></div>
                            <div className="flex justify-between items-center p-3 bg-surface-muted rounded-lg"><span>Address</span><span className="font-semibold text-right whitespace-pre-line">{address}</span></div>
                            <div className="flex justify-between items-center p-3 bg-surface-muted rounded-lg"><span>Contact</span><span className="font-semibold text-right">{contactInfo}</span></div>
                            <div className="flex justify-between items-center p-3 bg-surface-muted rounded-lg"><span>Payment Info</span><span className="font-semibold text-right whitespace-pre-line">{paymentDetails}</span></div>
                        </div>
                         <div className="text-xs p-3 border rounded-lg h-24 overflow-y-auto bg-surface-muted">
                        ...
                        <div className="flex gap-3">
                             <button type="button" onClick={handleBack} className="flex-1 bg-surface-sunken text-ink font-bold py-4 rounded-xl">Back</button>
                            <button onClick={handleSubmit} disabled={!termsAccepted} className="flex-1 bg-success text-white font-bold py-4 rounded-xl disabled:bg-gray-400 shadow-lg">Complete Setup</button>
                        </div>
                    </div>
                );
            default: return null;
        ...
        <div className="bg-surface-muted min-h-screen font-sans">
             <header className="p-4 bg-surface-muted sticky top-0 z-10">
                <div className="flex justify-between items-center mb-4">
                    ...
                    <button onClick={onBack} className="text-sm font-semibold text-ink-soft bg-surface-sunken px-3 py-1 rounded-lg">Exit Setup</button>
                </div>
