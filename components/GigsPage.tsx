        ...
        <div className="fixed inset-0 bg-black bg-opacity-60 flex justify-center items-center z-50 p-4" onClick={onClose}>
            <div className="bg-white rounded-Nonecontrol shadow-xl p-6 w-full max-w-sm" onClick={e => e.stopPropagation()}>
                ...
                <form onSubmit={handleSubmit} className="space-y-4">
                    <div>
                        <label htmlFor="bidAmount" className="block text-body font-medium text-ink">Your Bid Amount ({gig.currency})</label>
                        <input
                            type="number"
                            id="bidAmount"
                            value={amount}
                    ...
                    <div>
                        <label htmlFor="proposal" className="block text-body font-medium text-ink">Your Proposal</label>
                        <textarea
                            id="proposal"
                            rows={4}
                            value={proposal}
...
const GigDetailView: React.FC<GigDetailViewProps> = ({ gig, provider, onBack, onSelectProvider, onApply, isAuthenticated, onAuthClick }) => {
    ...
    return (
        ...
        <div className="fixed inset-0 bg-black bg-opacity-60 flex justify-center items-end z-40" onClick={onBack}>
            <div 
                className="bg-surface-muted rounded-t-sheet shadow-xl w-full max-w-sm h-[85vh] flex flex-col" 
                onClick={e => e.stopPropagation()}
            >
                <div className="flex-shrink-0 relative">
                    <img src={gig.imageUrl} alt={gig.title} className="w-full h-48 object-cover rounded-t-sheet" />
                ...
                <div className="p-4 flex-1 overflow-y-auto space-y-4">
                    <span className="text-body font-bold text-brand-primary uppercase">{gig.category}</span>
                    ...
                    <p className="text-body text-ink mt-4 leading-relaxed whitespace-pre-wrap">{gig.description}</p>
                        ...
                        <div>
                            ...
                            <button onClick={() => onSelectProvider(provider)} className="flex items-center gap-3 p-3 bg-white border border-line rounded-Nonecontrol mt-2 w-full text-left hover:bg-surface-sunken">
                                ...
                                <div>
                                    ...
                                    <p className="text-body text-ink-soft">{provider.service}</p>
                                </div>
                            </button>
                        </div>
                    )}
        ...
        <div className="p-4 bg-surface-muted min-h-full">
            ...
            {gigs.length > 0 ? (
                        ...
                        return (
                            <button key={gig.id} onClick={() => setSelectedGig(gig)} className="w-full text-left bg-white p-3 rounded-Nonecard shadow-sm hover:shadow-md transition-shadow flex items-center gap-4 border border-line">
                                ...
                                <div className="flex-grow overflow-hidden">
                                    ...
                                    <p className="text-body font-semibold text-brand-dark mt-1">
                                        {gig.currency} {gig.budget.toLocaleString()}
                                        <span className="text-caption font-normal text-ink-soft"> {budgetTypeSuffix[gig.budgetType]}</span>
                                    </p>
                                    {provider && <p className="text-caption text-ink-soft mt-1">by {provider.name}</p>}
                                </div>
                ...
                <div className="text-center py-16 text-ink-soft">
                    ...
                    <h3 className="mt-2 text-body font-medium text-ink">No gigs available right now</h3>
                    <p className="mt-1 text-body text-ink-soft">Please check back later for new opportunities.</p>
                </div>
            )}
