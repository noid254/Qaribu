...
const ServiceCard: React.FC<{ provider: ServiceProvider; onClick: () => void }> = ({ provider, onClick }) => {
    return (
        <div onClick={onClick} className="card-interactive overflow-hidden w-full group duration-300">
            <div className="relative h-40">
                ...
                {provider.isVerified && (
                    <div className="absolute top-2 right-2 bg-info text-white rounded-full p-1 shadow-raised">
                        <VerifiedIcon className="w-4 h-4"/>
                    </div>
                )}
                <img src={provider.avatarUrl} alt={provider.name} className="absolute -bottom-6 left-3 w-12 h-12 rounded-full border-4 border-surface shadow-raised" />
                <div className="absolute bottom-2 right-2 bg-brand-navy-dark/70 backdrop-blur-sm text-white text-[11px] px-2 py-1 rounded-full font-semibold flex items-center gap-1">
            ...
            <div className="p-3 pt-8 space-y-1">
                <h3 className="font-bold text-base text-ink truncate">{provider.service}</h3>
                <div className="flex justify-between items-center">
                    <p className="text-body text-ink-soft truncate">{provider.name}</p>
                ...
                <p className="text-lg font-bold text-brand-navy pt-1">
                    {provider.currency}
                    {provider.hourlyRate.toLocaleString()}
                    <span className="text-body font-medium text-ink-faint">/{rateSuffix[provider.rateType]}</span>
                </p>
            </div>
        </div>
    );
