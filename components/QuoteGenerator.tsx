    ...
    <div className="bg-surface-sunken min-h-screen font-sans">
       <div className="p-4 bg-white sticky top-0 z-10 shadow-sm border-b">
          <div className="flex justify-between items-center">
            <button onClick={onBack} className="text-ink-soft">&times;</button>
                ...
                {lineItems.map(item => (
                    <div key={item.id} className="flex justify-between items-center p-2 bg-surface-muted rounded">
                        ...
                        <div className="flex items-center gap-4">
                            ...
                            <button onClick={() => removeItem(item.id)} className="text-danger font-bold text-xl">&times;</button>
                        </div>
                    </div>
                ))}
            </div>
        ...
        <div>
            <div ref={ref} className="bg-white p-8 rounded shadow-lg max-w-4xl mx-auto border border-line font-sans text-body" style={{ width: '210mm', minHeight: '297mm', margin: 'auto' }}>
                ...
                <header className="flex justify-between items-start pb-4 mb-8">
                    <div className="flex items-center gap-4">
                        {assets.logo && <div className="w-20 h-20 rounded-full bg-surface-sunken flex items-center justify-center overflow-hidden"><img src={assets.logo} alt="logo" className="max-w-full max-h-full object-contain"/></div>}
                        <h1 className="text-4xl font-bold text-ink tracking-wide">QUOTE</h1>
                    </div>
                ...
                <section className="grid grid-cols-3 gap-8 mb-8 text-caption">
                    <div className="col-span-1">
                     ...
                     <div className="col-span-1">
                        <h3 className="font-bold text-ink-soft uppercase tracking-wider mb-2">PREPARED BY</h3>
                        <p className="font-semibold text-ink">{fromName}</p>
                        <p className="whitespace-pre-line text-ink-soft">{fromDetails}</p>
                    </div>
                </section>
                    ...
                    <tbody>
                        {lineItems.map((item: LineItem) => (
                            <tr key={item.id} className="border-b border-line">
                                <td className="py-2 pr-4 font-semibold text-ink">{item.description}</td>
                                <td className="py-2 px-4 text-center text-ink-soft">{item.quantity}</td>
                                <td className="py-2 px-4 text-right text-ink-soft">{currencyFormatter.format(item.unitPrice)}</td>
                                <td className="py-2 pl-4 text-right font-semibold text-ink">{currencyFormatter.format(item.quantity * item.unitPrice)}</td>
                            </tr>
                        ))}
                    </tbody>
                </table>
                ...
                <section className="grid grid-cols-2 gap-8">
                    ...
                    <div className="text-right">
                        <div className="space-y-2 text-body">
                            ...
                            {shipping > 0 && <div className="flex justify-between"><span className="text-ink-soft">Shipping</span><span>{currencyFormatter.format(shipping)}</span></div>}
                            <div className="flex justify-between font-bold text-base border-t-2 border-gray-800 pt-2 mt-2"><span className="text-ink">Total</span><span>{currencyFormatter.format(totalDue)}</span></div>
                        </div>
                    </div>
                </section>
